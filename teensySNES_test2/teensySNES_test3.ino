...
const int cintMovementThreshold = 5; //5

//The average zero acceleration values read
//from the accelerometer for each axis:
const int cintZeroXValue = 335; //335
const int cintZeroYValue = 333; //333
const int cintZeroZValue = 407; //407 - Z Not mapped

//The maximum (positive) acceleration values read
//from the accelerometer for each axis:
const int cintMaxXValue = 404; //404
const int cintMaxYValue = 401; //401
const int cintMaxZValue = 675; //675 - Z Not mapped

//The minimum (negative) acceleration values read
//from the accelerometer for each axis:
const int cintMinXValue = 267;  //267
const int cintMinYValue = 267;  //267
const int cintMinZValue = 140; //66  - Z Not mapped
...
const int cintMaxMouseMovement = 20; //20
...
void setup()
{
  ...
  //Uncomment this line for debugging. Also uncomment required section below:
//  Serial.begin(9600);
}

void loop()
{
  //Uncomment this line for debugging. (add a linefeed to debugging output)
//  Serial.println();
  ...
  // Light LED if any button is pressed - button debugging
  digitalWrite ( pinLEDOutput, AnyKeysPressed);
  // After LED is set, reset variable for next button pass
...
void fcnProcessAccelerometer()
{
  ...
  //Debug Accelerometer Output - Uncomment Below
//  Serial.print ("  X:");
//  Serial.print (intAnalogXReading);
//  Serial.print ("  Y:");
//  Serial.print (intAnalogYReading);
//  Serial.print ("  Z:");
//  Serial.print (intAnalogZReading);
...
//    intMouseXMovement = cintXSign * ( ( ( (float)( 2 * cintMaxMouseMovement ) / ( cintMaxXValue - cintMinXValue ) ) * ( intAnalogXReading - cintMinXValue ) ) - cintMaxMouseMovement );
    //it could use some improvement, like making it trigonometric. (trig version below)
  if( intAnalogXReading < cintMinXValue ) { intAnalogXReading = cintMinXValue; }
  if( intAnalogXReading > cintMaxXValue ) { intAnalogXReading = cintMaxXValue; }
  
 intMouseXMovement = cintXSign * ( float( ( ( tan( ( ( intAnalogXReading - cintMinXValue ) * ( 130.000 / ( cintMaxXValue - cintMinXValue ) ) -65.000 ) * 3.142 / 180 ) ) ) * 10.000 / 21.400 ) * cintMaxMouseMovement );
...
//    intMouseYMovement = cintYSign * ( ( ( (float)( 2 * cintMaxMouseMovement ) / ( cintMaxYValue - cintMinYValue ) ) * ( intAnalogYReading - cintMinYValue ) ) - cintMaxMouseMovement );
    //it could use some improvement, like making it trigonometric. (trig version below)
  if( intAnalogYReading < cintMinYValue ) { intAnalogYReading = cintMinYValue; }
  if( intAnalogYReading > cintMaxYValue ) { intAnalogYReading = cintMaxYValue; }
  
 intMouseYMovement = cintYSign * ( float( ( ( tan( ( ( intAnalogYReading - cintMinYValue ) * ( 130.000 / ( cintMaxYValue - cintMinYValue ) ) -65.000 ) * 3.142 / 180 ) ) ) * 10.000 / 21.400 ) * cintMaxMouseMovement );
  }
  else
  {
  ...
  //Debug Mouse Movement - Uncomment Below
//  Serial.print ("  Mouse X:");
//  Serial.print (intMouseXMovement);
//  Serial.print ("  Y:");
//  Serial.print (intMouseYMovement);
}
...
{ 
  ...
  for (byte i = 0; i < NUMBUTTONS; i++) {
    ...
    {                              //this button is pressed
      keysPressed = true;
      AnyKeysPressed = true;    // Set LED Variable on
